...
module.exports = app => {
  ...
  app.post('/event', passport.authenticate('jwt', {session: false}), (req, res) =>{
    ...
    const { title, groupId, amount, isPayment, frequency, website, category, date, groupEndDate, notes } = req.body
    Event.create({ title, groupId, amount, isPayment, frequency, website, category, date, groupEndDate, notes, author })
      .then(event => {
